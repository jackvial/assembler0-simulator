...
class SimulatedRobot:
    ...
    def read_position(self) -> np.ndarray:
        ...
        """
        # Return positions for all joints (5 for 5DOF, 6 for 6DOF)
        num_joints = min(6, len(self.d.qpos))  # Support both 5DOF and 6DOF
        return self.d.qpos[:num_joints]
    ...
    def inverse_kinematics(self, ee_target_pos, joint_name='end_effector'):
        ...
        num_joints = len(qpos)  # Adapt to 5DOF or 6DOF
        qdot = np.dot(np.linalg.pinv(jac[:, :num_joints]), ee_target_pos - ee_pos)
